...
It supersedes relying on fragmented chat history and should be treated as the
primary briefing document for future chats. Other `.md` files remain valuable as
deeper reference documents, but this file should describe:
...
40KArmy is a lightweight web tool that estimates the real-world retail cost of
...
The project has now moved beyond MVP validation and into a serious
...
The app is functioning as a real public product and has already received
meaningful traffic and community interest.
...
`unit.id` → `kit-mappings/{faction}.json` → `kits/{faction}.json` → enrich unit
in UI
...
If a Forge World or Legends unit has no standard retail mapping, it may still be
unpriced. That is acceptable.
...
This prevents stale or conflicting retail data from living in the gameplay
layer.

### B. The app now enriches units at runtime from mappings + kits

This permanently solved the earlier bug where correctly mapped units still
appeared AWOL.
...
Once retail kits are mapped, remaining units are reviewed and labeled where
appropriate as:
...
This is important because earlier manual scraping attempts were slow and
unreliable. Mike’s approach uses an actual browser automation workflow, allowing
structured kit data to be pulled directly from GW.
...
Slug should be derived from the GW product URL slug, but normalized into the
project’s stable slug format.
...
This avoids duplication of the entire generic marine range across every chapter
file.
...
- Mike has already completed at least one scraper-generated faction file
  (Sororitas) and indicated multiple additional factions are in progress
- this contributor workflow is still actively being discussed in the dedicated
  data chat
- this document only records that the scraper path now exists and may become the
  main acceleration route for faction kit population
...
These factions were manually swept through using the new architecture and
runtime enrichment logic.
...
The dedicated data chat remains the active place for discussing live faction
rollout and Mike coordination. This document records only the latest known state
...
It now appears near the main CTA area and is intended as a soft voluntary
support mechanism.
...
These edits were fiddly and time-consuming, but the desktop view is now in a
stronger place.
...
The mobile UI spec for sticky totals remains part of the active product
direction.

The architecture and pricing logic must not be modified by future mobile UI
work.
...
The key fix in this phase was ensuring these labels reflect the cleaned runtime
enrichment logic rather than stale unit-level price data.
...
This is significant because it suggests the project can grow through search over
time, not just Reddit spikes.
...
The project is no longer purely speculative. It has real audience validation and
the main job now is improving completeness and usefulness.
...
There is / was a faction dropdown duplication issue (e.g. Custodes appearing
twice). This is cosmetic and not a blocker, but should be fixed in core build
work.

## B. Some local-vs-live quirks

A few temporary local-state / stale-cache oddities happened during development,
but the live build generally reflected the correct logic after deployment.

## C. Still incomplete faction coverage

The biggest product limitation remains incomplete coverage. The project becomes
much more compelling as more factions are completed.
...
4. Runtime enrichment must resolve: `unit.id -> mapping -> kit`
5. AWOL should only mean: no mapping exists
...
To avoid context fragmentation, the project should be split into only a few
clear work chats:
...
- the product has real signs of usage, validation, and future monetisation
  potential
...
If the scraper workflow holds, the entire project timeline compresses
significantly.
